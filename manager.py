...
class DBManager:
    def __init__(self, db_name=DB_NAME):
        ...
        self._create_table()
        self._upgrade_db()  # Run the "safety net" every time the app starts
    ...
    def get_all_pcs(self):
        self.cursor.execute("SELECT * FROM pcs ORDER BY alias, hostname")
...
class PCManager:
    ...
    # Updated version: No sudo, more accurate counting
    ...
    def setup_ui(self):
        ...
        # 2. Create the Button in setup_ui
    ...
    def _get_current_time_str(self, mask="%Y-%m-%d %H:%M:%S"):
        ...
        if file_path:
            try:
                # 2. Get all text from the CORRECT widget: self.log_view
    ...
    def load_pc_data(self):
        ...
        # 1. First, build the color map (Essential for colors to work)
        ...
        # 2. Setup the UI tags based on the map we just built
        self._setup_log_tags()
        self._setup_treeview_tags()

        # 3. Handle dynamic height
        num_pcs = len(raw_pcs)
        treeview_height = max(4, min(15, num_pcs))
        self.pc_list_view.config(height=treeview_height)

        # 4. Insert data with the correct color tags
        for idx, row in enumerate(raw_pcs):
            # Safety extraction (handles 7, 8, or 10 columns)
            if len(row) < 7:
                continue
    ...
    # Add this at the very bottom of the PCManager class
    def export_log(self):
    ...
    def _run_ssh_command(self, pc_info, command):
        ...
        try:
            ...
            # Check if connection was lost during a reboot (this is expected)
            if any(x in command for x in ['reboot', 'shutdown']):
                if any(msg in str(e).lower() for msg in ['reset by peer', 'timed out', 'not established', 'eof']):
                    self.log_message(f"[SSH OK] {alias}: Connection dropped (Expected for reboot).")
    ...
    def _check_pending_updates_count(self, pc_info):
        ...
        cmd = "apt list --upgradable 2>/dev/null | grep -c upgradable || echo 0"
        success, output = self._run_ssh_command(pc_info, cmd)
    ...
    def _check_status_data(self, pc_info):
        ...
        """
        # We combine both into one string to save SSH connection time
