...
class PCManager:
    ...
    def _auto_idle_check(self):
        ...
        if self._fleet_sync_in_progress:
            ...
            # FIX: Reschedule and then RETURN (the return must be inside this IF)
        ...
        # 5. Schedule next normal check 5 minutes from NOW
        self._idle_check_timer_id = self.master.after(
        self.AUTO_CHECK_INTERVAL_MS, self._auto_idle_check
        )
    ...
    def _reboot_monitor(self, pc_info, action):
        ...
        try:
            ...
            # 2. Update status to Rebooting/Shutting Down
            # We use 'after' to let the Main Thread handle the DB update to avoid recursion errors
            ...
            # Phase 1: Wait for it to drop offline
            time.sleep(10) 
        ...
        def action_worker():
            ...
            try:
                ...
                for pc in pc_list:
                    ...
                    new_status = "OK" if success else f"Failed: {status_msg}"
                    current_time = self._get_current_time_str()
